...
#define SERVER_ADDRESS "172.0.0.1"

int main(int argc, char** argv){
    char *port, *serveraddr; //Range from 0-65535 so five digits is always sufficient
    port = DEFAULT_PORT;
    ...
    //knowninfo.ai_addr = address;// Use localhost for now. TODO: change this to accept an argument
    if(getaddrinfo(serveraddr, port, &knowninfo, &clientinfo) != 0) {
    ...
    connect(socketfd, clientinfo->ai_addr, clientinfo->ai_addrlen);
    return 0;
}
